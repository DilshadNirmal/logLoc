...
const TabGroup = ({ tabOptions, variant = "horizontal" }) => {
  ...
  return (
    <div
      className={`${
        variant === "vertical"
          ? "flex flex-col justify-between h-full p-4 md:p-2 2xl:p-4 py-8 md:py-3 2xl:py-8"
          : "grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-2.5 2xl:gap-4 mt-4 md:mt-3 m-4 md:m-2 2xl:mx-6 2xl:my-4"
      } `}
    >
      {tabOptions.map((tab) => (
        <TabButton
