...
export const RadioOption = ({
  ...
  <label className="flex items-center gap-3 cursor-pointer text-text">
    <div className="relative">
      ...
      />
      <div
        className={`w-3.5 h-3.5 md:w-4 md:h-4 2xl:w-5 2xl:h-5 border-2 ${
          checked ? "border-primary bg-primary/20" : "border-primary"
        } rounded-full flex items-center justify-center`}
      >
        {checked && (
          <div className="w-2 h-2 md:w-2 md:h-2 2xl:w-3 2xl:h-3 rounded-full bg-primary"></div>
        )}
      </div>
    </div>
...
export const DownloadButton = ({ onClick }) => (
  <button
    onClick={onClick}
    className="flex items-center justify-center gap-2 p-2 px-4 md:p-3 md:px-6 bg-primary text-white rounded-md md:rounded-lg hover:bg-primary/90 transition-colors"
  >
